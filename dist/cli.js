...
async function handleAccounts(args) {
    ...
    switch (action) {
        ...
        case "add": {
            ...
            if (!creds)
                error("No credentials configured. Run: gccli accounts credentials <credentials.json>");
...
async function handleCalendars(account) {
...
async function handleAcl(account, args) {
    const calendarId = args[0];
    if (!calendarId)
        error("Usage: <email> acl <calendarId>");
...
async function handleEvents(account, args) {
    ...
    if (!calendarId)
        error("Usage: <email> events <calendarId> [options]");
    const now = new Date();
...
async function handleEvent(account, args) {
    const calendarId = args[0];
    const eventId = args[1];
    if (!calendarId || !eventId)
        error("Usage: <email> event <calendarId> <eventId>");
...
async function handleCreate(account, args) {
    const { values, positionals } = parseArgs({
        args,
        options: {
            summary: { type: "string" },
    ...
    const calendarId = positionals[0];
    if (!calendarId)
        error("Usage: <email> create <calendarId> --summary <s> --start <dt> --end <dt>");
...
async function handleUpdate(account, args) {
    const { values, positionals } = parseArgs({
        args,
        options: {
            summary: { type: "string" },
    ...
    if (!calendarId || !eventId)
        error("Usage: <email> update <calendarId> <eventId> [options]");
...
async function handleDelete(account, args) {
    const calendarId = args[0];
    const eventId = args[1];
    if (!calendarId || !eventId)
        error("Usage: <email> delete <calendarId> <eventId>");
    const event = await service.getEvent(account, calendarId, eventId);
    const details = [
        `Calendar: ${calendarId}`,
        `Event: ${eventId}`,
